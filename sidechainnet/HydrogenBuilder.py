...
ALERT: Builds HIS for all protonation states (HIP, HIE, HID) 
"""
import math

import numpy as np
import torch
from sidechainnet.build_info import BB_BUILD_INFO, HYDROGEN_NAMES, NUM_COORDS_PER_RES, SC_BUILD_INFO
from sidechainnet.structure import coord_generator
from sidechainnet.sequence import ONE_TO_THREE_LETTER_MAP, GLOBAL_PAD_CHAR, ATOM_MAP_14
...
class HydrogenBuilder(object):
    ...
    def __init__(self, seq, coords):
        ...
        self.device = coords.device if self.mode=="torch" else None
    ...
    def M(self, axis, theta):
        ...
        out = [[aa + bb - cc - dd, 2 * (bc + ad), 2 * (bd - ac)],
               [2 * (bc - ad), aa + cc - bb - dd, 2 * (cd + ab)],
               [2 * (bd + ac), 2 * (cd - ab), aa + dd - bb - cc]]
        # TODO better way to deal with numpy/torch & device
        out_array = self.array(out, device=self.device) if self.mode=='torch' else self.array(out)
        return out_array
    ...
    def pad_hydrogens(self, resname, hydrogens):
        ...
        one = self.ones(3)
        if self.mode =="torch": one = torch.ones(3, device=self.device)
        pad_vec = [GLOBAL_PAD_CHAR * one]
        n_heavy_atoms = sum(
