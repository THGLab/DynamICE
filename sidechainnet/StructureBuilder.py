...
from sidechainnet.sequence import ONE_TO_THREE_LETTER_MAP, VOCAB
from sidechainnet.build_info import SC_BUILD_INFO, BB_BUILD_INFO, NUM_COORDS_PER_RES, SC_ANGLES_START_POS, NUM_ANGLES
from sidechainnet.HydrogenBuilder import HydrogenBuilder, NUM_COORDS_PER_RES_W_HYDROGENS
from sidechainnet.structure import nerf
...
class StructureBuilder(object):
    ...
    def _build_first_two_residues(self):
        ...
        first_res = ResidueBuilder(first_resname, first_ang, device=self.device, 
                                   prev_res=None, next_res=None)
    ...
    def _initialize_coordinates_and_PdbCreator(self):
        ...
        if not self.pdb_creator:
            from sidechainnet.PdbBuilder import PdbBuilder
            if self.data_type == 'numpy':
...
class ResidueBuilder(object):
    ...
    def __init__(self,
        ...
        if isinstance(angles, np.ndarray):
            angles = torch.tensor(angles, device=torch.device("cpu"), dtype=torch.float32)
        self.name = name
        ...
        self.sc = []
        self.coords = []
        self.coordinate_padding = torch.zeros(3, device=self.device, requires_grad=True)
    ...
    def build_bb(self):
        ...
        if self.prev_res is None:
            ...
            for j in range(4):
                ...
                pb = torch.tensor(pb, device=self.device)
                next_pt = nerf(pts[-3],
                               pts[-2],
                               pts[-1],
                               b,
    ...
    def _init_bb(self):
        ...
        o = nerf(
            ...
            l_bc=torch.tensor(BB_BUILD_INFO["BONDLENS"]["ca-c"], device=self.device),  # Previous bond length
            nerf_method=self.nerf_method)
        return [n, ca, c, o]
...
def _get_residue_build_iter(res, build_dictionary, device):
    ...
    pbond_vals = [torch.tensor(BB_BUILD_INFO['BONDLENS']['n-ca'], dtype=torch.float32,
                               device=device)] + [torch.tensor(b, dtype=torch.float32, 
                               device=device) for b in r["bonds-vals"]][:-1]
